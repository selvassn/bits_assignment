...
const characterDetailsQuery = gql`query MyQuery($id: ID!) {
  character(id: $id) {
  id
  name
  status
...
const getCharacterDetails = async(id: number) => {
  ...
  const { character } = await graphQLClinent.request(characterDetailsQuery, variables);
  return character;
}

export function useGetCharacters(isEnabled: boolean, search: ICharacterFilter) {
  return useQuery(["get-characters", search], async () => getCharacters(search), {enabled: isEnabled});
}
